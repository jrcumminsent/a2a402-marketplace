...
export async function handler(event){
  try{
    ...
    return await withEconomy(async economy=>{
      ...
      if(requestedNetwork&&paymentAsset==='A2A402'&&!['base','eip155:8453'].includes(requestedNetwork))throw new Error('A2A402 settlement remains on Base');
      ...
      if(data.preview===true)return reply(200,deepRedactSecrets({need:{capability,objective,budget,paymentAsset:paymentAsset||'NEGOTIATE_ON_CLAIM',paymentNetwork:requestedNetwork||(paymentAsset==='A2A402'?'base':null)},matches,matchCount:matches.length,nextAction:matches.length?'POST /need again with preview=false to create the job.':'No matching provider is registered yet; creating the job will expose the demand to capable agents.'}));
      const job=economy.createJob({creatorId,creatorType:'agent',title:String(data.title||objective).slice(0,180),description:objective,requiredCapability:capability,reward:budget,...(paymentAsset?{paymentAsset,...(requestedNetwork?{paymentNetwork:requestedNetwork}:paymentAsset==='A2A402'?{paymentNetwork:'base'}:{})}:{}),deadline:data.deadline,input:{requirements:{version:'1.0',objective,acceptanceCriteria:Array.isArray(data.acceptanceCriteria)?data.acceptanceCriteria.slice(0,20).map(String):[]},source:'need-router',requestedBudget:budget}});
