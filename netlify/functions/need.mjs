...
export async function handler(event){
  try{
    ...
    return await withEconomy(async economy=>{
      ...
      const rawAsset=data.paymentAsset?String(data.paymentAsset).trim().toUpperCase():null;const requestedAsset=rawAsset==='A2A402'?'A2A':rawAsset;
      if(requestedAsset&&!['A2A','USDC'].includes(requestedAsset))throw new Error('paymentAsset must be A2A or USDC');
      ...
      const paymentAsset=requestedAsset||(declaredAssets.has('A2A')||declaredAssets.has('A2A402')?'A2A':declaredAssets.has('USDC')?'USDC':null);
      const requestedNetwork=data.paymentNetwork?String(data.paymentNetwork).trim().toLowerCase():null;
      const usdcNetworks=new Set(['base','ethereum','arbitrum','optimism','polygon','eip155:8453','eip155:1','eip155:42161','eip155:10','eip155:137']);
      if(requestedNetwork&&paymentAsset==='USDC'&&!usdcNetworks.has(requestedNetwork))throw new Error('unsupported USDC paymentNetwork');
      if(requestedNetwork&&paymentAsset==='A2A'&&!['base','eip155:8453'].includes(requestedNetwork))throw new Error('A2A settlement remains on Base');
      const matches=economy.searchAgents({requiredCapability:capability,maxPrice:budget,minimumReputation:Number(data.minimumReputation||0)}).filter(match=>!isInternalAgent(economy.agents.get(match.agentId))).slice(0,10);
      if(data.preview===true)return reply(200,deepRedactSecrets({need:{capability,objective,budget,paymentAsset:paymentAsset||'NEGOTIATE_ON_CLAIM',paymentNetwork:requestedNetwork||(paymentAsset==='A2A'?'base':null)},matches,matchCount:matches.length,nextAction:matches.length?'POST /need again with preview=false to create the job.':'No matching provider is registered yet; creating the job will expose the demand to capable agents.'}));
      const job=economy.createJob({creatorId,creatorType:'agent',title:String(data.title||objective).slice(0,180),description:objective,requiredCapability:capability,reward:budget,...(paymentAsset?{paymentAsset,...(requestedNetwork?{paymentNetwork:requestedNetwork}:paymentAsset==='A2A'?{paymentNetwork:'base'}:{})}:{}),deadline:data.deadline,input:{requirements:{version:'1.0',objective,acceptanceCriteria:Array.isArray(data.acceptanceCriteria)?data.acceptanceCriteria.slice(0,20).map(String):[]},source:'need-router',requestedBudget:budget}});
