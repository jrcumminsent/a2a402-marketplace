...
const tokenAddress=(process.env.A2A402_TOKEN_ADDRESS||process.env.A2A402_EXPECTED_TOKEN_ADDRESS||'0xF2bb6DC14E9097EC08F9Eaa9C6B7d39662195F01').trim();
...
export async function handler(event){try{if(event.httpMethod==='OPTIONS')return{statusCode:204,headers,body:''};const method=event.httpMethod,p=pathFor(event);return await withEconomy(async economy=>{
  if(method==='GET'&&p==='/payments/execution/genesis/intents'){
    ...
    return reply(200,{protocol:'a2a402-genesis-payment-execution-v1',environment:'production',realMoney:true,persistence:persistenceMode(),custody:false,signer:'metamask-agent-wallet',creatorId:genesisCreator,payerAddress:genesisPayer,intents});
  }
  const match=p.match(/^\/payments\/execution\/genesis\/jobs\/([^/]+)\/settle$/);
  if(method==='POST'&&match){const job=economy.jobs.get(match[1]);if(!job)return reply(404,{error:{code:'NOT_FOUND',message:'job not found'}});if(!isGenesis(job))return reply(403,{error:{code:'NOT_GENESIS',message:'job is not eligible for Genesis autonomous settlement'}});if(job.status!=='AWAITING_PAYMENT')return reply(409,{error:{code:'INVALID_STATE',message:'job is not awaiting payment'}});assertPayer(job);if(!ADDRESS.test(job.payeeAddress||''))throw new Error('worker payout wallet is not configured');const data=parseBody(event);const worker=await verifyTransfer({txHash:data.workerTxHash,from:genesisPayer,to:job.payeeAddress,amountUnits:job.workerPaymentUnits});const fee=await verifyTransfer({txHash:data.feeTxHash,from:genesisPayer,to:treasuryAddress,amountUnits:job.marketplaceFeeUnits});const settled=economy.settleA2AJob(job.id,genesisCreator,{worker,fee});return reply(200,{protocol:'a2a402-genesis-payment-execution-v1',settled:true,job:settled});}
