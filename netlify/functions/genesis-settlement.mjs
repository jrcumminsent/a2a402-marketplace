...
async function verifyTransfer({txHash,from,to,amountUnits}){if(!HASH.test(txHash||''))throw new Error('valid Base Mainnet transaction hash required');if(!ADDRESS.test(from||'')||!ADDRESS.test(to||''))throw new Error('valid payer and recipient required');const receipt=await rpc('eth_getTransactionReceipt',[txHash]);if(!receipt)throw new Error('transaction is not mined yet');if(receipt.status!=='0x1')throw new Error('transaction failed on Base Mainnet');const match=(receipt.logs||[]).find(log=>{if(String(log.address||'').toLowerCase()!==tokenAddress.toLowerCase())return false;if(String(log.topics?.[0]||'').toLowerCase()!==transferTopic)return false;if(topicAddress(log.topics?.[1])!==from.toLowerCase()||topicAddress(log.topics?.[2])!==to.toLowerCase())return false;try{return BigInt(log.data||'0x0')===BigInt(amountUnits)}catch{return false}});if(!match)throw new Error('transaction does not contain the required A2A transfer');return{txHash,from,to,amountUnits:String(amountUnits),network:'base',chainId:8453,asset:'A2A',tokenAddress}}
...
export async function handler(event){try{if(event.httpMethod==='OPTIONS')return{statusCode:204,headers,body:''};const method=event.httpMethod,p=pathFor(event);return await withEconomy(async economy=>{
  if(method==='GET'&&p==='/payments/execution/genesis/intents'){
    const intents=[...economy.jobs.values()].filter(job=>isGenesis(job)&&job.status==='AWAITING_PAYMENT'&&job.paymentAsset==='A2A'&&job.paymentNetwork==='base').map(job=>{assertPayer(job);return paymentIntentForJob(job,{baseUrl,tokenAddress,treasuryAddress})});
    return reply(200,{protocol:'a2a402-genesis-payment-execution-v1',environment:'production',realMoney:true,persistence:persistenceMode(),custody:false,signer:'metamask-agent-wallet',creatorId:genesisCreator,payerAddress:genesisPayer,asset:'A2A',tokenAddress,intents});
  }
  const match=p.match(/^\/payments\/execution\/genesis\/jobs\/([^/]+)\/settle$/);
  if(method==='POST'&&match){const job=economy.jobs.get(match[1]);if(!job)return reply(404,{error:{code:'NOT_FOUND',message:'job not found'}});if(!isGenesis(job))return reply(403,{error:{code:'NOT_GENESIS',message:'job is not eligible for Genesis autonomous settlement'}});if(job.status!=='AWAITING_PAYMENT')return reply(409,{error:{code:'INVALID_STATE',message:'job is not awaiting payment'}});if(job.paymentAsset!=='A2A'||job.paymentNetwork!=='base')return reply(409,{error:{code:'INVALID_ASSET',message:'job is not configured for Base Mainnet A2A settlement'}});assertPayer(job);if(!ADDRESS.test(job.payeeAddress||''))throw new Error('worker payout wallet is not configured');const data=parseBody(event);const worker=await verifyTransfer({txHash:data.workerTxHash,from:genesisPayer,to:job.payeeAddress,amountUnits:job.workerPaymentUnits});const fee=await verifyTransfer({txHash:data.feeTxHash,from:genesisPayer,to:treasuryAddress,amountUnits:job.marketplaceFeeUnits});const settled=economy.settleA2AJob(job.id,genesisCreator,{worker,fee});return reply(200,{protocol:'a2a402-genesis-payment-execution-v1',settled:true,asset:'A2A',tokenAddress,job:settled});}
