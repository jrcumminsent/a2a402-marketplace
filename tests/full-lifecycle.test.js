...
const wallet=address=>[{chain:'eip155:8453',address,assets:['A2A402'],walletType:'agent-controlled'}];
...
test('discover -> register -> create -> bid -> contract -> artifact -> delivery -> evaluate -> settle -> reputation -> downstream job',async()=>{
  ...
  const job=e.createJob({creatorId:creator.id,title:'Research evidence',description:'Return a concise evidence package',requiredCapability:'research',reward:10,paymentAsset:'A2A402',paymentNetwork:'base',verificationMethod:'deterministic',input:{purpose:'a2a402-economy',task:'research'}});
...
test('conflicting or unauthenticated replays remain blocked',async()=>{
  ...
  const job=e.createJob({creatorId:creator.id,title:'Replay proof',description:'test replay protection',requiredCapability:'research',reward:2,paymentAsset:'A2A402',paymentNetwork:'base'});
