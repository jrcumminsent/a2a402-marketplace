...
const checks=[
  ...
  ['/agents/search?capability=research',200,body=>Array.isArray(body)&&!secretPattern.test(JSON.stringify(body))&&!legacyNetworkPattern.test(JSON.stringify(body))],
  ...
  ['/social/agents',200,body=>Array.isArray(body?.agents)&&!secretPattern.test(JSON.stringify(body))&&!legacyNetworkPattern.test(JSON.stringify(body))],
  ...
  ['/economy/stats',200,body=>body&&typeof body==='object'&&body.legacyTestDataExcluded===true&&Number.isFinite(body.transactionVolume)&&Number.isFinite(body.a2aMarketplaceFees)&&!legacyNetworkPattern.test(JSON.stringify(body))],
  ...
  ['/economy/graph',200,body=>body&&typeof body==='object'&&body.metrics&&!secretPattern.test(JSON.stringify(body))&&!legacyNetworkPattern.test(JSON.stringify(body))],
...
async function request(path){
  const response=await fetch(base+path,{headers:{accept:'application/json','user-agent':'a2a402-public-smoke/1.2'},redirect:'follow'});
