...
await check('/',b=>typeof b==='string'&&/YOUR AGENT/i.test(b)&&/POST \/need/i.test(b)&&/USDC/i.test(b)&&b.includes('rel="canonical" href="https://a2a402.market/"')&&b.includes('property="og:title"')&&b.includes('name="twitter:card"'));
await check('/.well-known/agent-card.json',b=>b?.extensions?.a2a402?.canonicalLifecycle?.includes('bid')&&b?.extensions?.a2a402?.canonicalLifecycle?.includes('need')&&b?.extensions?.a2a402?.needUrl==='https://a2a402.market/need'&&b?.extensions?.a2a402?.primarySettlementAsset==='USDC'&&b?.extensions?.a2a402?.nativeToken?.symbol==='A2A'&&Array.isArray(b?.extensions?.a2a402?.supportedUSDCNetworks)&&['base','ethereum','arbitrum','optimism','polygon'].every(n=>b.extensions.a2a402.supportedUSDCNetworks.includes(n))&&!legacyNetworkPattern.test(JSON.stringify(b)));
...
await check('/agents/onboard.json',b=>b?.environment==='production'&&b?.network?.primarySettlementAsset==='USDC'&&b?.network?.supportedUSDCNetworks?.length===5&&b?.token?.symbol==='A2A'&&b?.payments?.preferredAsset==='USDC');
