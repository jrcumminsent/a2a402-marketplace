...
must('public/index.html',/USDC\s+first/i,'USDC-first homepage settlement');
must('public/index.html',/A2A(?:<[^>]+>)*\s*optional/i,'A2A optional homepage settlement');
for(const network of ['Base','Ethereum','Arbitrum','Optimism','Polygon'])must('public/index.html',new RegExp(network,'i'),`homepage USDC network ${network}`);
