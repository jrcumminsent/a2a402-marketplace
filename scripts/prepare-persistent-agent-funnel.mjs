...
if (source.includes('action: recommendedJob ? "bid_on_recommended_job"')) {
  // Already upgraded.
} else if (source.includes(returnOld)) {
  source = source.replace(returnOld, returnNew);
} else {
  throw new Error("Could not locate persistent registration response block.");
}
