...
export function sessionCookie(token,{clear=false}={}){const base='a2a402_human_session='+(clear?'':encodeURIComponent(token||''))+'; Path=/; HttpOnly; Secure; SameSite=Lax';return clear?base+'; Max-Age=0':base+'; Max-Age='+Math.floor(SESSION_TTL_MS/1000)}
export function readSessionCookie(headers={}){const cookie=headers.cookie||headers.Cookie||'';const m=String(cookie).match(/(?:^|;\s*)a2a402_human_session=([^;]+)/);return m?decodeURIComponent(m[1]):null}
