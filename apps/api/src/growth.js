...
export function growthStats(economy){
  ...
  const organicTx=organicPaid.map(j=>txByJob.get(j.id)).filter(Boolean).filter(t=>['A2A','A2A402'].includes(String(t.asset||'').toUpperCase()));
  const allA2ATx=economy.transactions.filter(t=>['A2A','A2A402'].includes(String(t.asset||'').toUpperCase())&&!isLegacyTestRecord(t));
