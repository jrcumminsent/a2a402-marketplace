...
function migrateAgent(agent) {
  ...
  let supportedPayments = Array.isArray(agent.supportedPayments) ? agent.supportedPayments
  ...
  // Persisted production state may predate the Base Mainnet bootstrap wallet.
  // Repair only the built-in broker; never invent or modify wallets for external agents.
  if (agent.id === BOOTSTRAP_CREATOR) {
    const hasMainnetA2A = wallets.some(wallet => wallet.chain === MAINNET_CHAIN && Array.isArray(wallet.assets) && wallet.assets.map(String).map(x=>x.toUpperCase()).includes('A2A'));
    if (!hasMainnetA2A) {
      wallets = [...wallets, {
        id: `${BOOTSTRAP_CREATOR}:base-mainnet-bootstrap`,
        chain: MAINNET_CHAIN,
    ...
    if (!supportedPayments.some(payment => String(payment.asset || '').toUpperCase() === 'A2A' && payment.network === MAINNET_CHAIN)) {
      supportedPayments = [{network:MAINNET_CHAIN,asset:'A2A',primary:true,marketplaceFeeBps:500}, ...supportedPayments];
    }
...
function migrateJob(job) {
  ...
  if (job.paymentAsset === 'A2A' && job.paymentNetwork === 'base-sepolia' && ['OPEN','CLAIMED','IN_PROGRESS','SUBMITTED','VERIFYING'].includes(job.status)) {
    migrated = { ...job, paymentNetwork:'base', paymentRoute:null, payerAddress:null, payeeAddress:null, status:'OPEN', workerId:null, claimedAt:null, submittedAt:null, updatedAt:new Date().toISOString() };
  }
  if (migrated.creatorId === BOOTSTRAP_CREATOR && migrated.paymentAsset === 'A2A' && migrated.paymentNetwork === 'base' && ['OPEN','IN_PROGRESS','SUBMITTED','VERIFYING','AWAITING_PAYMENT'].includes(migrated.status)) {
    migrated = { ...migrated, payerAddress:A2A402_TREASURY };
  }
  return migrated;
}
...
export async function withEconomy(fn, { baseUrl = 'https://a2a402.market', loungeEnabled = true } = {}) {
  ...
  try {
    ...
    const existing = await client.query(`SELECT state FROM ${stateTable} WHERE id = 1 FOR UPDATE`);
    ...
    await client.query(
      `INSERT INTO ${stateTable} (id, state, updated_at)
