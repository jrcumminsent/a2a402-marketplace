...
export const CONTRACT_SCHEMAS: Readonly<Record<string, ContractJsonSchema>> = {
  ...
  SettlementReceipt: objectSchema({
    id: UUID,
    version: stringSchema({ const: "a2a402-settlement-receipt/0.2" }),
    settlementId: UUID,
    jobId: UUID,
    contractId: UUID,
    buyerAgentId: UUID,
    ...
    asset: ASSET,
    network: stringSchema({ minLength: 1, maxLength: 64 }),
    assetContract: nullable(WALLET),
    paymentTransactionHash: stringSchema({
      minLength: 1,
      maxLength: 255,
    }),
    provenanceLotId: UUID,
    parentProvenanceLotIds: arraySchema(UUID),
    economicClassification: stringSchema({ const: "AGENT_EARNED" }),
    issuer: stringSchema({ minLength: 1, maxLength: 255 }),
    issuedAt: DATE_TIME,
    keyId: stringSchema({ minLength: 1, maxLength: 512 }),
    digest: SHA256_HEX,
    signature: stringSchema({ minLength: 1, maxLength: 2_048 }),
    verification: objectSchema({
      algorithm: stringSchema({ const: "Ed25519" }),
      canonicalization: stringSchema({ const: "a2a402-canonical-json/0.1" }),
    }),
  }),
...
export const HTTP_ROUTE_CONTRACTS: readonly HttpRouteContract[] = [
  ...
  route({
    id: "lightweight_agent_document",
    kind: "meta",
    method: "GET",
    path: "/.well-known/agent.json",
    summary: "Lightweight autonomous-agent discovery document",
    description:
      "Compact, actionable discovery links for autonomous software agents. All economic activity is A2A_TEST simulation only.",
    response: componentSchemaRef("JsonObject"),
  }),
  route({
    id: "did_document",
  ...
  route({
    id: "autonomous_marketplace_discovery",
    kind: "meta",
    method: "GET",
    path: "/api/discovery",
    summary: "Autonomous marketplace discovery beacon",
    description:
      "Machine-readable capabilities, entry points, and explicit A2A_TEST non-real-money safety status.",
    response: componentSchemaRef("JsonObject"),
  }),
  route({
    ...
    summary: "List public test opportunities",
    description:
      "Anonymous discovery of the Genesis bounty and open marketplace jobs with no private agent or bid data.",
    response: componentSchemaRef("JsonObject"),
  }),
  route({
    id: "autonomous_agent_genesis_bounty",
    kind: "rest",
    method: "GET",
    path: "/api/bounties/autonomous-agent-genesis",
    summary: "Autonomous Agent Genesis test bounty",
    description:
      "Standing public A2A_TEST bounty instructions and attribution limitations. No fiat or mainnet value is offered.",
    response: componentSchemaRef("JsonObject"),
  }),
  route({
    id: "record_discovery_evidence",
    kind: "rest",
    method: "POST",
    path: "/api/discovery/evidence",
    summary: "Record privacy-limited discovery evidence",
    description:
      "Idempotently records explicit discovery attribution without cookies or invasive fingerprinting. Evidence does not prove autonomy.",
    security: "public-idempotent",
    body: objectSchema(
      {
        first_landing_endpoint: stringSchema({ minLength: 1, maxLength: 256 }),
        source: stringSchema({ minLength: 1, maxLength: 64 }),
        utm_source: stringSchema({ minLength: 1, maxLength: 128 }),
        agent_framework: stringSchema({ minLength: 1, maxLength: 128 }),
        discovery_document: stringSchema({ minLength: 1, maxLength: 256 }),
        self_reported_source: stringSchema({ minLength: 1, maxLength: 256 }),
      },
      [],
    ),
...
export function marketplaceManifest(input: {
  ...
  return {
    ...
    public_endpoints: {
      ...
      autonomous_agent_document: `${input.publicUrl}/.well-known/agent.json`,
      discovery: `${input.publicUrl}/api/discovery`,
      opportunities: `${input.publicUrl}/api/opportunities`,
      genesis_bounty: `${input.publicUrl}/api/bounties/autonomous-agent-genesis`,
